	...
	switch(isset($url[1]) ? $url[1] : null) 
	{
		...
		case 'social_auth':	
				...
				if (!($user_auth==false))
				{
					...
					{					
						...
						else
						{
							...
							{
								$no_head = true;
								$access_auth = $db->getRow($db->query("SELECT `nick`, `group` FROM `" . USER_DB . "`.`" . USER_PREFIX . "_users` WHERE provider = '" . $db->safesql($user_auth->provider) . "' AND social_id = '" . $db->safesql($user_auth->socialId) . "'"));	
								...
								else
								{
									...
									$core->tpl->redicret('Вход не выполнен! Администраторам запрещено авторизоваться через социальные сети!', '/profile', 'Ошибка!');
								}
							}					
						}
					}
			...
			{					
					unset($_SESSION['user_auth']);	
					$no_head = true;
					$core->tpl->redicret('Вход не выполнен! Авторизоваться через эту социальную сеть запрещено!', '/profile', 'Ошибка!');
			}
